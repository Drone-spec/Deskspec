...
var rxs = [{"rx":/(https?:\/\/(?:.+\.)+(?:googleusercontent|blogspot)\.com\/(?:.+\/){4})(.+)\/(.+)/, "rep":"$1d/$3"}, {"rx":/(https?:\/\/(?:.+\.)+(?:ggpht)\.com\/(?:.+\/){4})(.+)/, "rep":"$1d/$3"}, {"rx":/https?:\/\/farm\d+\.staticflickr\.com\/\d+\/(\d+).*/, "type":"flikr", "rep":"$1"}];
function ImageData(data) {
  $.extend(this, data);
  if (this["data"]["LensID"] && EXIF["Lens"][this["data"]["Make"]["data"]] && EXIF["Lens"][this["data"]["Make"]["data"]][this["data"]["LensID"]["data"]]) {
    this["data"]["LensModel"] = $.extend(exifAttributes["LensModel"], {"data":EXIF["Lens"][this["data"]["Make"]["data"]][this["data"]["LensID"]["data"]], "label":chrome.i18n.getMessage("LensModel")});
  }
  this.histogram = true;
}
...
function getEmbeddedEXIF(src, callback, originalSrc) {
  ...
  EXIF.getData(oImage, function() {
    ...
    $.each(aExifData, function(name, tag) {
      ...
      if (data) {
        $.extend(tag, {"data":data, "label":chrome.i18n.getMessage(name) || name});
      } else {
        delete aExifData[name];
      }
    });
    var gps_data = {Latitude:EXIF.getTag(oImage, "GPSLatitude"), Longitude:EXIF.getTag(oImage, "GPSLongitude"), LatitudeRef:EXIF.getTag(oImage, "GPSLatitudeRef"), LongitudeRef:EXIF.getTag(oImage, "GPSLongitudeRef"), Position:EXIF.getTag(oImage, "GPSPosition")};
    var gps = {};
    gps["lat"] = Geo.parseDMS(gps_data.Latitude, gps_data.LatitudeRef);
    gps["lng"] = Geo.parseDMS(gps_data.Longitude, gps_data.LongitudeRef);
    callback(new ImageData({"data":aExifData, "gps":gps, "src":originalSrc, "originalSrc":src}));
  });
}
function getFlikrEXIF(id, callback, src) {
  var oData = {"src":src};
  $.when($.ajax({"url":"http://api.flickr.com/services/rest/", "dataType":"json", "data":{"method":"flickr.photos.getExif", "api_key":flickrApiKey, "photo_id":id, "format":"json", "nojsoncallback":"1"}, "success":function(data) {
    var aExifData = {};
    if (data["photo"]) {
      $.each(data["photo"]["exif"], function(i, tag) {
        var data = tag["raw"]["_content"], name = tag["tag"];
        if (parseInt(tag["tag"])) {
        ...
        if (typeof exifAttributes[name] != "undefined") {
          aExifData[name] = $.extend({}, exifAttributes[name], {"data":data, "label":chrome.i18n.getMessage(name)});
        } else {
          aExifData[name] = {"data":data, "label":tag["label"], "visible":false};
        }
      });
    }
    oData["data"] = aExifData;
  }}), $.ajax({"url":"http://api.flickr.com/services/rest/", "dataType":"json", "data":{"method":"flickr.photos.geo.getLocation", "api_key":flickrApiKey, "photo_id":id, "format":"json", "nojsoncallback":"1"}, "success":function(data) {
    if (data && data["photo"] && data["photo"]["location"]) {
      oData["gps"] = {"lat":data["photo"]["location"]["latitude"], "lng":data["photo"]["location"]["longitude"]};
    }
  }}), $.ajax({"url":"http://api.flickr.com/services/rest/", "dataType":"json", "data":{"method":"flickr.photos.getSizes", "api_key":flickrApiKey, "photo_id":id, "format":"json", "nojsoncallback":"1"}, "success":function(data) {
...
function genericOnClick(info, tab) {
  ...
  chrome.tabs.sendRequest(tab.id, {"action":"startExifProcessing", "data":info.srcUrl});
  checkExif(info.srcUrl, function(data) {
    chrome.tabs.sendRequest(tab.id, {"action":"showExif", "data":data});
  });
}
var context = "image";
...
chrome.extension.onRequest.addListener(function(request, sender, callback) {
  if (request["action"] == "checkExif") {
    ...
    if (request["action"] == "checkFlikrExif") {
      getFlikrEXIF(request["id"], callback);
    } else {
      if (request["action"] == "getSettings") {
        callback({"overlayEnabled":localStorage.getItem("overlayEnabled") != "false", "accordion":localStorage.getItem("accordion") ? JSON.parse(localStorage.getItem("accordion")) : [0, 1, 2, 3], "tab":0});
      } else {
        if (request["action"] == "setSeetings") {
          if (request["accordion"]) {
            localStorage.setItem("accordion", JSON.stringify(request["accordion"]));
          }
        } else {
          if (request["action"] == "checkShowCameraImage") {
            getCameraImage(request, callback);
          } else {
            if (request["action"] == "getHistogramData") {
...
function getCameraImage(camera, callback) {
  if (cameraImages[camera["model"]]) {
    ...
    $.when($.ajax({"url":"https://www.dpreview.com/products/search-by-name", "type":"POST", "data":{"terms":camera["model"].replace(/\n{2,}/g, "")}, "dataType":"json", "success":function(resp) {
      if (resp["result"] && resp["products"] && resp["products"][0]) {
        $.extend(cameraImages[camera["model"]], {"imageUrl":resp["products"][0]["imageUrl"], "productUrl":resp["products"][0]["productUrl"], "name":resp["products"][0]["name"], "model":camera["model"], "brand":camera["brand"], "tbUrl":resp["products"][0]["imageUrl"]});
        var veerTestVar = cameraImages[camera["model"]];
        veerTestVar = veerTestVar;
      }
    }})).then(function() {
      callback(cameraImages[camera["model"]]);
